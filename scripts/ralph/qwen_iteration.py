...
QWEN_MODEL = os.environ.get("QWEN_MODEL", "ralph-3ds")
...
SKIP_DIR_NAMES = {
    ...
    "build-host",
    "shots",
    "romfs",       # generated blobs/audio: built by make, never model-written
    "data",        # generated t3x atlas
    "docs",        # ops/audit docs: stale-doc hazard, never snapshotted (failure #26)
    "dist",
}
SKIP_COMMIT_PATHS = [
    "node_modules",
...
def next_story(prd: dict) -> dict | None:
    """Lowest-priority unpassed story in the CURRENT phase only (checkpoint gating)."""
    phase = current_phase()
    stories = [
        s
        for s in prd.get("userStories", [])
        if not s.get("passes") and str(s.get("phase", "0")) == phase
    ]
    if not stories:
        return None
...
def phase_complete(prd: dict) -> bool:
    """True when this phase is done but later-phase stories remain."""
    return next_story(prd) is None and not all_passed(prd)
...
def safe_rel(path: str) -> str | None:
    ...
    if parts[0] in SKIP_DIR_NAMES:
        return None
    # Write fence: the model may ONLY write app code and authored asset JSON.
    # tools/, tests/, scripts/, docs/, romfs/, data/, gfx/ binaries stay ours.
    if parts[0] not in {"source", "include", "assets", "gfx"}:
        print(f"  skip {rel}: outside source/include/assets/gfx write fence", flush=True)
        return None
    if parts[0] == "assets" and Path(rel).suffix not in {".json"}:
        print(f"  skip {rel}: only .json is writable under assets/", flush=True)
        return None
    if parts[0] == "gfx" and Path(rel).suffix not in {".t3s", ".ppm"}:
        print(f"  skip {rel}: only .t3s/.ppm writable under gfx/", flush=True)
        return None
    if "generated" in parts:
        print(f"  skip {rel}: generated headers are built by make, never written", flush=True)
        return None
    return rel
...
def ping() -> int:
    """Prove endpoint + prefill workaround: expect a fast literal PONG."""
    reply = chat("Reply with exactly: PONG-3ds. Nothing else.")
